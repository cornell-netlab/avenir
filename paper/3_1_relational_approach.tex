...
and $(\var{*}, \var{0x1})$. \todo[inline]{How do we compute $\kappa$?}
...
\todo[inline]{How to synthesize a sequence of edits for a single rule}

\subsubsection{Mapping Synthesis}


\todo[inline]{How do we do this?}
...
% \todo[inline]{Add motivating prose}
...
% The reason that we want the column range of each action is so that we
...
\todo[inline]{ In what follows we will make the assumption that the
  free variables in the actions of tables are bound in the same
  table's keys. Formally, for a table $(\vec{\var f}, \vec A, \Phi)$,
  we assume that
  $\bigcup_{A_i}\bigcup_{a \in A_i} \fvs(a) \subseteq \{\var f_1,
  \ldots, \var f_n\}$.}  \todo[inline]{We should prove this}
...
\todo[inline]{Redo Proof for intervals}
