...
\begin{figure*}
  \begin{tabular}{c c c l l}
    ...
    & $\forall \tau_l. \wp_{\tau_l}(c_l) \Leftrightarrow \wp_{f(\tau_l)}(c_r)$ \\
    \textsc{EditSynth}
    & n/a
    & $\wp_{\tau_l}(c_l) \Leftrightarrow \wp_{\tau_r}(c_r)$
    & $f : \mathsf{Edit} \to 2^{\mathsf{Edit}}$
    & $\forall \rho. \wp_{(\tau_l \otimes \rho)}(c_l)$ \\
    &&&&\qquad $\Leftrightarrow \wp\left(\hole\parenbb{c_r}_{\left(\tau_r\otimes \bigotimes f(\rho)\right)}\right)$ \\
    ...
    \textsc{BoundEditSynth}$(\oplus, s, n)$
    & $n$
    & $\wp_{\tau_l}(c_l) \Leftrightarrow \wp_{\tau_r}(c_r)$
    & $f_i : \Edit_s \to \Edit$,
    & $\forall \rho_s. \wp(s^\oplus\parenbb{c_l}_{(\tau_l \oplus \rho_s)})$\\
    &&&\hfill $i \in [n]$
    &\qquad $\Leftrightarrow \wp(\hole\parenbb{c_r}_{(\tau_r \otimes f_1(\rho) \cdots \otimes f_n(\rho))})$ \\
    \textsc{BoundEditVerif}$(\oplus, s, n)$
    & $f_i : \Edit_s \to \Edit$
    & $\wp_{\tau_l}(c_l) \Leftrightarrow \wp_{\tau_r}(c_r)$
    & \texttt{unsat}
    & \textit{sim.}\\
