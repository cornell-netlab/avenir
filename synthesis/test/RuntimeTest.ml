...
let bmv2_parser_parses_real_rules _ =
      ...
      [ ordered
          [ ( bigand
            ...
            , sequence
                ...
                ; apply
                    ( "forward"
                    , [("meta.routing_metadata.nhop_ipv4", 32)]
                    , [ ( "set_dmac"
                        , [("dmac", 48)]
                        , "hdr.ethernet.dstAddr" %<-% Var ("dmac", 48) )
                      ; ("drop", [], drop) ]
                    , drop ) ] )
          ; (True, Skip) ]
      ; apply
  ...
  let expected =
    ...
    [ Add
        ( "send_frame"
        , ( [Match.exact_ "standard_metadata.egress_port" (Value.make (1, 9))]
          , [Value.str_make ("0x00aabb000000", 48)]
          , 0 ) )
    ; Add
        ( "send_frame"
        , ( [Match.exact_ "standard_metadata.egress_port" (Value.make (2, 9))]
          , [Value.str_make ("0x00aabb000001", 48)]
          , 0 ) )
    ; Add
        ( "forward"
        , ( [ Match.exact_ "meta.routing_metadata.nhop_ipv4"
                (Value.str_make ("0x0a00000a", 32)) ]
          , [Value.str_make ("0x000400000000", 48)]
          , 0 ) )
    ; Add
        ( "forward"
        , ( [ Match.exact_ "meta.routing_metadata.nhop_ipv4"
                (Value.str_make ("0x0a00010a", 32)) ]
          , [Value.str_make ("0x000400000001", 48)]
          , 0 ) )
    ; Add
        ( "ipv4_lpm"
        , ( [ Match.exact_ "hdr.ipv4.dstAddr"
                (Value.str_make ("0x0a00000a", 32)) ]
  ...
  |> Equality.same_stringlist
       (List.map ~f:(Edit.to_bmv2_string phys) expected)

let test_bmv2 : unit Alcotest.test_case list =
  [ Alcotest.test_case "parses_real_rules" `Quick
      bmv2_parser_parses_real_rules ]
