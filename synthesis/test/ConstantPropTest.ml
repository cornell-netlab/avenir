...
let cp_wikipedia_ex1 _ =
  ...
  let cmd =
    sequence
      ...
      ; ( "y"
        %<-% Expr.(
               minus (times (value (2, 32)) (value (7, 32))) (Var ("x", 32)))
        )
      ; ( "return"
        %<-% Expr.(
               times
                 (Var ("y", 32))
                 (plus (Expr.value (99, 32)) (Expr.value (2, 32)))) ) ]
  in
  let exp =
    sequence
      [ "x" %<-% Expr.value (14, 32)
...
let cp_only_holes_and_constants _ =
  ...
  let cmd =
    sequence
      ...
      ; ordered
          [ ( Hole ("?Del_0_fwd_table", 1)
              %=% Expr.value (0, 1)
              %&% (Var ("ipv4.dst", 32) %=% Expr.value (3333, 32))
            , "out_port" %<-% Expr.value (45, 9) )
          ; ( Hole ("?AddTo_fwd_table", 1)
              %=% Expr.value (1, 1)
      ...
      ; Apply
          ...
          ; actions=
              [ ( "action"
                ...
                , sequence
                    [ ( "out"
                      %<-% Expr.(plus (Var ("port", 9)) (cast 9 @@ var "z"))
                      )
                    ; "meta" %<-% Expr.plus (var "z") (var "z") ] )
              ; ( "action"
                , [("port", 9)]
                , sequence
                    ["out" %<-% Expr.value (0, 9); "meta" %<-% int (2 * 99)]
                ) ]
  ...
  let exp =
    sequence
      ...
      ; Apply
          { name= "tbl"
          ; keys=
              [ Key.(set_val (make ("z", 32)) (Value.make (99, 32)))
              ; Key.make ("addr", 32) ]
          ; actions=
              [ ( "action"
                , [("port", 9)]
                , sequence
                    [ ( "out"
                      %<-% Expr.(plus (Var ("port", 9)) (Expr.value (99, 9)))
                      )
                    ; "meta" %<-% int 198 ] )
              ; ( "action"
                , [("port", 9)]
                , sequence ["out" %<-% Expr.value (0, 9); "meta" %<-% int 198]
                ) ]
...
let passive_propogation_learns_from_tests1 _ =
  ...
  let cmd =
    sequence
      ...
      ; assume
          (Var ("standard_metadata.egress_spec$2", 9) %=% Expr.value (1, 9))
      ]
  in
  let map =
  ...
  | Some e -> same_expr (Expr.value (1, 9)) e
  ...
  let cmd =
    ordered
      [ ( bigand
        ...
        , sequence
            [ assume
                (Hole ("?dst_addr_0_ipv4_rewrite", 48) %=% Expr.value (0, 48))
            ; assume
                ( Var ("standard_metadata.egress_spec$2", 9)
                %=% Expr.value (1, 9) ) ] )
      ; ( bigand
            [ Hole ("?AddRowToipv4_fwd", 1) %=% Expr.value (1, 1)
...
let test : unit test_case list =
  ...
  ; test_case "only holes, constants, and output vars remain in tests" `Quick
      cp_only_holes_and_constants
  ; test_case "cp_also_props_copies" `Quick cp_also_props_copies
  ; test_case "opt_bcm_example" `Quick opt_bcm_example
  ; test_case "cp_affects_actions_and_keys" `Quick
      cp_affects_actions_and_keys ]

let test_passive_prop : unit test_case list =
  [ test_case "learns from tests 1" `Quick
      passive_propogation_learns_from_tests1
  ; test_case "learns from tests 2" `Quick
      passive_propogation_learns_from_tests2
