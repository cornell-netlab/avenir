    ...
    sequence
      ...
      ; mkOrdered
          [ ( Hole ("?Del_0_fwd_table", 1)
              %=% mkVInt (0, 1)
              %&% (Var ("ipv4.dst", 32) %=% mkVInt (3333, 32))
            , "out_port" %<-% mkVInt (45, 9) )
...
let rev_propogate_computes_single_path _ =
  ...
  let out_packet =
    Util.StringMap.of_alist_exn
      [("port$3", mkVInt (99, 32)); ("ipv4.dst$1", mkVInt (5, 32))]
  in
  same_cmd
    ( mkAssume
        (bigand
...
let test : unit test_case list =
  ...
  ; test_case "passive handwritten example" `Quick
      rev_propogate_computes_single_path ]
