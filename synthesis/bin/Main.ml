...
let opt_params : Parameters.t Command.Param.t =
  ...
  [%map_open
   ...
   and semantic_slicing = flag "-S" no_arg ~doc:"Do semantic slicing, -s required"
   and edits_depth = flag "-e" (required int) ~doc:"E maximum number of physical edits"
   and search_width = flag "-b" (required int) ~doc:"B maximum number of attempts per CE"
   and monotonic = flag "-m" no_arg ~doc:"[DEPRECATED] Prune rows with no holes"
   and injection = flag "--inj" no_arg ~doc:"Try injection optimization"
   and fastcx = flag "--fastcx" no_arg ~doc:"Generate counterexample quickly"
   and vcache = flag "--cache-queries" no_arg ~doc:"Enable query caching"
   and ecache = flag "--cache-edits" (optional int) ~doc:"N Enable edit caching when there are at least N previous successes"
   and shortening = flag "--shortening" no_arg ~doc:"shorten queries"
   and above = flag "--above" no_arg ~doc:"synthesize new edits above existing instance, not below"
   and minimize = flag "--min" no_arg ~doc:"try and eliminate each edit in turn"
   ...
   and nlp = flag "--nlp" no_arg ~doc:"[DEPRECATED] variable name based domain restrictions"
   and unique_edits = flag "--unique-edits" no_arg ~doc:"Only one edit allowed per table"
   and domain = flag "--domain-restrict" no_arg ~doc:"Restrict allowed values to those that occur in the programs"
   and restrict_mask = flag "--restrict-masks" no_arg ~doc:"Restrict masks"
   and no_defaults = flag "--no-defaults" no_arg ~doc:"Prefer solutions that don't rely on default actions"
   and no_deletes = flag "--no-deletes" no_arg ~doc:"try with no deletions"
   and use_all_cexs = flag "--use-all-cexs" no_arg ~doc:"attempt to use information from all cexs to compute models"
   and reach_restrict = flag "--reach-restrict" no_arg ~doc:"ensure all rules generated from model are reachable by the counterexample"
   and reach_filter = flag "--reach-filter" no_arg ~doc:"remove unreachable rules generated by model"
   and hot_start =  flag "--hot-start" no_arg ~doc:"process all the insertions to populate the caches, then rerun, and measure the second run"
       in
...
let mng_params =
  ...
  [%map_open
   ...
   and thrift_mode = flag "--thrift" no_arg ~doc:"parse & write bmv2/thrift commands"
   and interactive = flag "-i" no_arg ~doc:"interactive mode"
   and timeout = flag "--timeout" (optional float) ~doc:"Optional timeout in seconds"
   and prover_loc = flag "--loc" (optional string) ~doc:"<fp> Path to SMTLIB location, defaults to /usr/bin/z3" in
       Option.value prover_loc ~default:"/usr/bin/z3"
       |> Prover.make_provers;
       Parameters.({default with
                     debug;
                     thrift_mode;
...
let problem_flags =
  ...
  [%map_open
   ...
   and log_incl = flag "-I1" (listed string) ~doc:"<dir> add directory to include search path for logical file"
   and phys_incl = flag "-I2" (listed string) ~doc:"<dir> add directory to include search path for physical file"
   and data = flag "-data" (optional string) ~doc:"The logical experiment to run"
   and onos = flag "-onos" no_arg ~doc:"Parse logical edits as onos insertions" in
       ...
       let phys = if p4
                  then Encode.encode_from_p4 phys_incl physical false
                       |> Encode.unify_names var_mapping
                       |> Benchmark.zero_init fvs
                       |> Benchmark.drop_handle fvs
                  else Benchmark.parse_file physical in
       ...
       let data_to_synthesize =
         Option.map data ~f:(fun d ->
             if onos
             then Benchmark.(onos_to_edits var_mapping d "ipv6" "hdr.ipv6.dst_addr")
             else parse_runtime log d |> List.(map ~f:return)
           ) in
       (params,
        Problem.make ~log ~phys ~log_inst ~phys_inst ~log_edits:[] ~fvs ~phys_drop_spec,
        data_to_synthesize)
    ...
    [%map_open
     ...
     and monitor_props = flag "--monitor" (optional string) ~doc:"<name> Comma-separated list of properties to monitor" in
         ...
         if measure then
           ...
           let mk_prob =
             let open Problem in
             make ~log:(log prob) ~phys:(phys prob)
               ~log_inst:(log_inst prob) ~phys_inst:(phys_inst prob) ~log_edits:data
               ~fvs:(fvs prob) ~phys_drop_spec:(phys_drop_spec prob)
           in
           let prob = mk_prob () in
           if params.debug then
             Core.Printf.printf "PROBLEM: %s \n" (Problem.to_string params prob);
           match Synthesis.cegis_math_sequence params (ProfData.zero ()) mk_prob with
...
let encode_cmd : Command.t =
  ...
  Command.basic
    ...
    [%map_open
     let includes = flag "-I" (listed string) ~doc:"<dir> add directory to include search path"
...
let classbench_cmd : Command.t =
  ...
  Command.basic
    ...
    [%map_open
     let opt = opt_params
     and mng = mng_params
     and data = flag "-data" (required string) ~doc:"path to classbench data"
...
let ntbls_cmd : Command.t =
  ...
  Command.basic
    ...
    [%map_open
     ...
     and breadth = flag "-breadth" no_arg ~doc:"Breadth experiment when on, Length when not" in
         fun () ->
         let params = Parameters.union opt mng in
         if breadth then
...
let obt : Command.t =
  ...
  Command.basic
    ...
    [%map_open
     ...
     and incl = flag "-I" (listed string) ~doc:"<dir> add director to include search path for logical file" in
         fun () ->
         let prog =
           if p4 then
             Encode.encode_from_p4 incl prog_path false
           else
...
let server_cmd : Async_command.t =
  let open Command.Let_syntax in
    Command.async
      ...
      [%map_open
       let params, mk_prob, data = problem_flags <*> (map2 opt_params mng_params ~f:Parameters.union) in
           fun () ->
           assert (Option.is_none data);
