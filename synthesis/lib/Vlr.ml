...
module Make(V:HashCmp)(L:Lattice)(R:Result) = struct
  ...
  let rec to_string t =
    if t = drop then "DROP" else
      if t = id then "SKIP" else
  ...
  let mk_branch ((v,l) as test) tru fls =
    (* When the ids of the diagrams are equal, then the diagram will take on the
       ...
       a new id. *)
    (* Printf.printf "mk_branch (%s,%s) (%s @ %d) (%s @ %d)\n"
     *   (V.to_string v) (L.to_string l) (tru |> to_string) tru (fls |> to_string) fls; *)
    if equal tru fls then
      ((* Printf.printf "(%s = %s) %s @ %d and %s @ %d are identical\n"
        *   (V.to_string v) (L.to_string l) (tru |> to_string) tru (fls |> to_string) fls; *)
      fls)
    else if L.is_top l then tru else
      match unget fls with
  ...
  let apply f zero ~(cache: (t*t, t) Hashtbl.t) =
    let rec sum x y =
      let xy = Hashtbl.find_or_add cache (x, y) ~default:(fun () -> sum' x y) in
      (* Printf.printf "%s + %s = %s\n" (x |> to_string) (y |> to_string) (xy |> to_string); *)
      xy
    and sum' x y =
      ...
      | Branch {test=(vx, lx); tru=tx; fls=fx; all_fls=all_fls_x},
        ...
        |  0 ->
          ...
          |  0 -> 
              (* Printf.printf "%s = %s\n" (L.to_string lx) (L.to_string ly); *)
              mk_branch (vx,lx) (sum tx ty) (sum fx fy)
          | -1 ->             
             (* Printf.printf "%s < %s\n" (L.to_string lx) (L.to_string ly); *)
             mk_branch (vx,lx) (sum tx all_fls_y) (sum fx y)
          |  1 ->
              (* Printf.printf "%s > %s\n" (L.to_string lx) (L.to_string ly); *)
              mk_branch (vy,ly) (sum all_fls_x ty) (sum x fy)
          |  _ -> assert false
          end
  ...
  let clear_cache ~(preserve : Int.Set.t) =
    ...
    let preserve =
      ...
      |> fun init -> Int.Set.fold init ~init ~f:(fun init root ->
        List.fold (children root) ~init ~f:Int.Set.add
      )
    in
    begin
  ...
  let cond v t f =
    (* Printf.printf "cond (%s,%s) %d %d\n" (fst v |> V.to_string) (snd v|> L.to_string) t f; *)
    let ok t =
      match unget t with
      | Leaf _ -> true
  ...
  let render ?(format="pdf") ?(title="FDD") t =
    Util.compile_dot ~format ~title (to_dot t)
