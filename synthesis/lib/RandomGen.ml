...
let rec gen_keys width curr max =
  if curr >= max then
    ...
    failwith @@ Printf.sprintf "Cannot generate variable with negative index %d" curr
  else
    gen_key width curr :: gen_keys width (curr + 1) max
...
let rec gen_acts width curr max =
  if curr >= max then
    ...
    failwith @@ Printf.sprintf "Cannot generate action with negative index %d" curr
  else
    gen_act width curr :: gen_acts width (curr+1) max
...
let rec rand_matches width curr num_keys =
  if curr >= num_keys then
    ...
    failwith @@ Printf.sprintf "cannot generate negatively indexed key %i" curr
  else
    rand_match width curr :: rand_matches width (curr + 1) num_keys
...
module Pipe = struct
  let gen_staging bitwidth num_keys =
    ...
    let name = "staging" in
    let keys = gen_keys bitwidth 0 num_keys in
    let actions = ["set_meta", [("m",bitwidth)], "meta" %<-% Var ("m",bitwidth)] in
    let default = "meta" %<-% Value (Value.zero bitwidth) in
    Cmd.Apply {name;keys;actions;default}
