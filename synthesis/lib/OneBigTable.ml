...
let combine_actions (act1 : string * (string * int) list * Cmd.t)
    (act2 : string * (string * int) list * Cmd.t) :
    string * (string * int) list * Cmd.t =
...
let rec mk_one_big_table' (tbl : only_apply) c =
  ...
  | Assign _ ->
      { tbl with
        actions=
          List.map tbl.actions ~f:(fun (n, p, act) -> (n, p, act %:% c))
  ...
  | Apply app_t ->
      ...
      in
      let def_tbl_to_app_t =
        List.map app_t.actions
          ~f:(combine_actions ("DEFAULT", [], tbl.default))
      in
