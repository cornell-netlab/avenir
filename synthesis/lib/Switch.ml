...
type t =
  ...
  ; (* table rules *)
    edits: Edit.t list
  ; (* edits to table rules *)
    (* cached stuff *)
    gcl: Cmd.t option ref
  ; (* switch program with table rules inlined *)
    gcl_holes: Cmd.t option ref
  ; (* switch program with table rules inlined and holes added *)
    dels: Instance.interp option ref
  ; (* ??? *)
    tag: [`Mask | `Exact] option ref
  ; (* flag determining whether holes can be masked *)
    edited_inst: Instance.t option ref
  ; (* inst with edits applied *)
    do_slice: bool
  ; drop_spec: Test.t option (* dead code? *) }

let make ?(drop_spec = None) pipeline inst edits : t =
...
let to_gcl (params : Parameters.t) (_ : (string * int) list) (p : t) =
  if p.do_slice then
    ...
    if params.semantic_slicing then
      ...
      let slice =
        StaticSlicing.rule_slice params
          (edited_instance params p)
          (edits p) (pipeline p)
      in
      let c =
    ...
    | None ->
        let i =
          Instance.apply params NoHoles `Exact
            (edited_instance params p)
            !(p.pipeline)
        in
        p.gcl := Some i ;
...
let to_gcl_holes params (p : t) dels tag =
  ...
  | Some i, Some d, Some t
    when Instance.interp_equal d dels && Stdlib.(t = tag) ->
      i
  | _ ->
      let i =
        Instance.apply params ~no_miss:false dels tag
          (edited_instance params p)
          !(p.pipeline)
      in
      p.gcl_holes := Some i ;
...
let replace_edits (p : t) (edits : Edit.t list) =
  {p with edits} |> clear_cache

let append_edits (p : t) (edits : Edit.t list) =
  {p with edits= p.edits @ edits} |> clear_cache
...
let replace_inst (p : t) (i : Instance.t) =
  {p with inst= ref i} |> clear_cache

let slice_old params p =
  let inst_slice = Instance.of_edits params p.edits in
  Instance.overwrite (inst p) inst_slice |> replace_inst p

let slice_new p = {p with do_slice= true}

let slice params = if new_slicing then slice_new else slice_old params

let replace_pipeline (p : t) (c : Cmd.t) =
  {p with pipeline= ref c} |> clear_cache
