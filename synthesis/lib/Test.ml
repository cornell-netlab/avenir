...
let rec to_string t =
  ...
  | Impl (assum, conseq) -> "(" ^ to_string assum ^ " ==> " ^ to_string conseq ^ ")\n"
  | Iff (left, right) -> "(" ^ to_string left ^ " <==> " ^ to_string right ^ ")\n"
...
let rec and_ a b =
  if enable_smart_constructors then begin
      if equals a b then a else
        match a, b with
        | True, x | x, True -> x
        | False, _ | _, False -> False
        | Eq(Var x,u), Neg(Eq(Var y,v)) when Stdlib.(x = y)
          -> if Expr.equals u v then False
             else Eq(Var x,u)
        | _, And (b1, b2) -> (* left-associative *)
            and_ (and_ a b1) b2
        | _ -> And (a, b)
    end
  else And(a,b)
...
let impl assum conseq =
  if not enable_smart_constructors then Impl(assum, conseq) else
    if equals assum conseq then True else
...
let iff lhs rhs =
  ...
  then if equals lhs rhs then True else Iff(lhs, rhs)
  else Iff(lhs, rhs)
...
let rec num_nodes t =
  ...
  | True | False -> 1
  | Eq (left, right) ->
     Expr.num_nodes left + Expr.num_nodes right + 1
  | Le (left, right) ->
     Expr.num_nodes left + Expr.num_nodes right + 1
