...
let between_ key lo hi =
  if Value.equals lo hi then exact_ key lo
  else mk_match key @@ Between (lo, hi)
...
let to_string (m : t) : string =
  match m.data with
  | Exact x -> Printf.sprintf "%s ~ %s" m.key (Value.to_string x)
  | Between (lo, hi) ->
      Printf.sprintf "%s ~ [%s,%s]" m.key (Value.to_string lo)
        (Value.to_string hi)
  | Mask (v, msk) ->
      Printf.sprintf "%s ~ %s & %s" m.key (Value.to_string v)
        (Value.to_string msk)

let to_bmv2_string (m : t) : string =
  match m.data with
  | Exact v -> Value.to_bmv2_string v
  | Mask (v, m) ->
      Printf.sprintf "%s &&& %s" (Value.to_bmv2_string v)
        (Value.to_bmv2_string m)
  | _ ->
      Printf.sprintf
        "[Unimplemented] Don't know how to install range matches"
      |> failwith
...
let to_test (m : t) : Test.t =
  ...
  let k = (m.key, sz) in
  match m.data with
  | Exact v -> Var k %=% Value v
  | Between (lo, hi) ->
      let upper_bound = Value.big_make (max_int sz, sz) in
      let lo_test =
        if Value.leq lo (Value.zero sz) then True
        else Value lo %<=% Var (m.key, sz)
      in
      let hi_test =
        if Value.leq upper_bound hi then True else Var k %<=% Value hi
      in
      lo_test %&% hi_test
  | Mask (v, m) -> Expr.(mask (Var k) (Value m) %=% Value v)
...
let to_model_alist tbl m =
  match m.data with
  | Exact v -> [(Hole.match_hole_exact tbl m.key, v)]
  | Between (l, h) ->
      let loh, hih = Hole.match_holes_range tbl m.key in
      [(loh, l); (hih, h)]
  | Mask (vint, mask) ->
...
let test_hole_of_lists tbl ms =
  let open Test in
  List.fold ms ~init:True ~f:(fun acc m -> acc %&% to_test_hole tbl m)

let list_to_model_alist tbl ms = List.bind ms ~f:(to_model_alist tbl)

let to_model ?(typ = `Vals) table (m : t) =
  ...
  | Exact v ->
      ...
      Hole.match_holes_mask table m.key |> encode_msk (v, msk)
  | Mask (v, msk) -> Hole.match_holes_mask table m.key |> encode_msk (v, msk)
  | Between (lo, hi) ->
      let hlo, hhi = Hole.match_holes_range table m.key in
      [(hlo, lo); (hhi, hi)]

let list_to_model tbl ms = list_to_model_alist tbl ms |> Model.of_alist_exn
...
let to_valuation_test table hole_typ mtch =
  ...
  | `Mask, Mask (v, m) ->
      ...
      bigand [Hole (hv, sz) %=% Value v; Hole (hm, sz) %=% Value m]
  | `Mask, Exact v ->
      let hv, hm = Hole.match_holes_mask table k in
      bigand
        [ Hole (hv, sz) %=% Value v
  ...
  | _, Between _ ->
      failwith
        "[Match.valuation] Between Holes are impossible, so cannot create \
         such a valuation"
  | `Exact, Mask _ ->
      failwith
        "[Match.valuation] Mask valuation is incompatible with exact hole \
         encoding"

let is_wildcard m =
  match m.data with
  | Exact _ -> false
  | Mask (_, m) -> Bigint.(Value.get_bigint m = zero)
  | Between (lo, hi) ->
      let sz = Value.size hi in
      Bigint.(
        Value.get_bigint lo <= zero && Value.get_bigint hi >= max_int sz)
...
let mk_ipv6_match key ipv6_str =
  let addr_str, prefix_len =
    ...
    | None -> (ipv6_str, 128)
    | Some (addr, len) -> (addr, int_of_string len)
  in
  let hex_addr =
    let exp_addr_str =
  ...
  let data =
    if prefix_len = 128 then Exact (Value.big_make (bv, 128))
    else
      let mask =
        Bigint.of_string
          ( "0b"
          ^ String.make (prefix_len / 4) 'f'
          ^ String.make ((128 - prefix_len) / 4) '0' )
      in
      let hi =
        Bigint.(
          (bv land mask)
          + Bigint.shift_left Bigint.one Int.(128 - prefix_len)
          - Bigint.one)
      in
...
let cap (m : t) (m' : t) =
  if String.(m.key <> m'.key) then []
    ...
    | Exact x, Exact y -> if Value.eq x y then [m] else []
    | Exact x, Between (lo, hi) | Between (lo, hi), Exact x ->
        if Value.leq lo x && Value.leq x hi then [m] else []
    | Between (lo, hi), Between (lo', hi') ->
        let lo'' = Stdlib.max lo lo' in
        let hi'' = Stdlib.min hi hi' in
        if Value.leq lo'' hi'' then [{m with data= Exact lo''}]
        else if Value.leq lo'' hi'' then [{m with data= Between (lo'', hi'')}]
        else []
    | Mask _, _ | _, Mask _ -> failwith "Dont know how to intersect masks"

let rec has_inter_data (d : match_data) (d' : match_data) : bool =
  match (d, d') with
  | Exact x, Exact y -> Value.eq x y
  | Exact x, Between (lo, hi) | Between (lo, hi), Exact x ->
      Value.leq lo x && Value.leq x hi
  | Between (lo, hi), Between (lo', hi') ->
      Value.leq (Stdlib.max lo lo') (Stdlib.min hi hi')
  | Mask (v, msk), Exact v' | Exact v', Mask (v, msk) ->
      Bigint.(
        Value.get_bigint v land Value.get_bigint msk
        = Value.get_bigint v' land Value.get_bigint msk)
...
let has_inter (m : t) (m' : t) : bool =
  String.(m.key = m'.key)
  && (is_wildcard m || is_wildcard m' || has_inter_data m.data m'.data)

let has_inter_l (ms : t list) (ms' : t list) : bool =
  List.fold2_exn ms ms' ~init:true ~f:(fun acc m m' -> acc && has_inter m m')
...
let is_subset_data (d : match_data) (d' : match_data) : bool =
  ...
  | Between (lo, hi), Between (lo', hi') ->
      Value.leq lo hi' && Value.leq lo' hi
  | Mask _, _ | _, Mask _ -> failwith "Dont know how to subset masks"
  ...
  if is_wildcard m then m
    ...
    { m with
      data=
        ...
        | Between (lo, hi) ->
            if Bigint.(Value.get_bigint lo = zero) then Exact hi
            else Exact lo ) }
...
let hits mtch v =
  match mtch.data with
  | Exact v' -> Value.eq v v'
  | Mask (v', msk) -> Value.eq v (Value.mask v' msk)
  | Between (lo, hi) -> Value.leq lo v && Value.leq v hi
