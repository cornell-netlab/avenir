...
let rec equals c1 c2 =
  ...
  | Skip,Skip ->
     true
  | Assign (f1,e1), Assign(f2,e2) ->
     String.(f1 = f2) && Expr.equals e1 e2
  | Assume t1, Assume t2 ->
     Test.equals t1 t2
  | Seq (c1,c1'), Seq (c2,c2') ->
     equals c1 c2 && equals c1' c2'
...
and action_equals (name1, args1, a1) (name2, args2, a2) =
  String.(name1 = name2)
  && List.equal var_equals args1 args2
  && equals a1 a2
...
let assume t =
  if Test.(equals t True) then Skip else Assume t
...
let total ss =
  if not enable_smart_constructors then
    Select(Total, ss)
  else if List.length ss = 0 then
      assume False
    else
      Select (Total, ss)
...
let ordered ss =
  if not enable_smart_constructors then Select(Ordered, ss) else
    ...
    let selects =
      List.remove_consecutive_duplicates ss
        ~which_to_keep:`First
      ...
      |> List.fold ~init:([],true) ~f:(fun (cases,reachable) (b,c) ->
             if reachable then
               (cases @ [b,c], not(Test.equals b True))
             else
               (cases, reachable)
           )
      |> fst
    in
    if List.for_all selects ~f:(fun (_,c) -> equals c Skip) then
      Skip
    else
      match selects with
      | [] -> Skip
...
let rec to_string ?depth:(depth=0) (e : t) : string =
  match e with
...
let rec to_sexp_string e : string =
  let string_select = concatMap 
                        ~f:(fun (cond,act) -> "(" ^ Test.to_sexp_string cond ^ "," ^ to_sexp_string act ^ ")")
  ...
  | Apply t ->
     "Apply("
     ^ t.name ^ ",["
     ^ List.fold_left t.keys ~init:"" ~f:(fun str k -> Printf.sprintf "%s;\"%s\"" str (Key.to_string k))
     ^ "],["
     ^ List.fold_left t.actions ~init:"" ~f:(fun str a -> str ^ ";((SOME ACTION DATA), " ^ to_sexp_string (trd3 a) ^")")
...
let rec tables (c:t) : string list =
  ...
  | Select (_, cs) -> concatMap cs ~c:(@) ~init:(Some []) ~f:(fun (_, c) -> tables c)
  | Apply t -> [t.name]
...
let rec num_nodes c =
  ...
  | Select (_, cases) -> List.fold cases ~init:0 ~f:(fun acc (b, c) -> acc + Test.num_nodes b + num_nodes c)
...
let rec multi_vals c : (Value.t) list =
  ...
  | Apply t ->
     List.fold t.actions ~init:(multi_vals t.default)
       ~f:(fun rst act -> rst @ multi_vals (trd3 act))
    ...
    -> Apply {name=t.name;
              ...
              actions = List.map t.actions ~f:(fun (n, data, act) ->
                            let holes' = List.filter holes ~f:(fun h ->
                                             List.for_all (List.map data ~f:fst) ~f:(String.(<>) h)
                                           ) in
...
let rec get_schema_of_table name phys =
  ...
  | Apply t
    -> if String.(name = t.name) then Some (t.keys, t.actions, t.default) else None
...
let table_vars ?keys_only:(keys_only=false) ((keys : Key.t list), acts, default) =
  let open List in
  free_keys keys
  @ if keys_only then
      []
