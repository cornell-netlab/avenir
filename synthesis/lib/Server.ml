...
let member_not_null field json =
  ...
  member field json |> null_check (sprintf "Missing field: '%s'" field)
...
let parse_verify_request json =
  let open Yojson.Basic.Util in
  let parse_op op =
    match op |> member_not_null "opCode" |> to_string with
    | "ADD" -> parse_add_request op
    | "DELETE" -> parse_delete_request op
    | _ -> failwith "Something went wrong while parsing verify list"
  in
  let abstract_ops =
    json |> member "abstract_ops" |> to_list |> List.map ~f:parse_op
  in
  let physical_ops =
    json |> member "physical_ops" |> to_list |> List.map ~f:parse_op
  in
...
let edit_of_op prog op =
  ...
  | Add (table, matches, actionData, actId) -> (
    match Cmd.get_schema_of_table table prog with
...
let avenir_stub op : op list =
  ...
  let es = Synthesis.cegis_math !params (ProfData.zero ()) !problem in
  match es with
  | None -> failwith "Synthesis returned None"
  | Some es ->
...
let verify_stub abstract_ops physical_ops =
  let problem_ =
    ...
    @@ List.map abstract_ops ~f:(edit_of_op (Problem.log !problem))
  in
  let problem_ =
    Problem.append_phys_edits problem_
    @@ List.map physical_ops ~f:(edit_of_op (Problem.phys !problem))
  in
...
let handle_op_request (op : op) =
  (* some magic occurs *)
  let ops = avenir_stub op |> List.map ~f:op_to_json in
  `List ops
...
let process_post_request body =
  try
    ...
    | Verify (absOps, physOps) ->
        ...
        |> Yojson.Basic.to_string ~std:true
  with Failure e ->
    `Assoc [("PhysOps", `List []); ("OK", `Bool false); ("Error", `String e)]
    |> Yojson.Basic.to_string ~std:true

let handle_request ~(body : Cohttp_async.Body.t) (_ : Socket.Address.Inet.t)
...
let log_error _ exn = printf "Error: %s \n" (Exn.to_string exn)

let listen port () =
  Cohttp_async.Server.create ~on_handler_error:(`Call log_error)
