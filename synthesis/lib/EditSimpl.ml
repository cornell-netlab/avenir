...
let remove_missed_edits params data problem es =
  List.fold es ~init:[] ~f:(fun acc e ->
      ...
      let check =
        Test.(
          Packet.to_test ~fvs:(Problem.fvs problem) (fst cex)
          %=>% FastCX.hits_pred params data (Problem.phys problem)
...
let extract_reached_edits (params : Parameters.t) data problem model =
  ...
  let es =
    if params.reach_filter then remove_missed_edits params data problem es
    else es
  in
  if List.is_empty es then None else Some es
