...
let parse_ip_mask ident str =
  ...
  | Some len_str ->
      ...
      if len = 32 then Match.exact_ ident (Value.big_make (addr, 32))
      else
        let mask =
          Bigint.of_string
          @@ Printf.sprintf "0b%s%s" (String.make len '1')
               (String.make (32 - len) '0')
        in
        let lo_addr = Bigint.(addr land mask) in
        Match.mask_ ident
          (Value.big_make (lo_addr, 32))
          (Value.big_make (mask, 32))
...
let parse_port_range ident str =
  ...
  if Bigint.(lo_int = hi_int) then
    Match.exact_ ident (Value.big_make (lo_int, 16))
  else
    Match.between_ ident
      (Value.big_make (lo_int, 16))
      (Value.big_make (hi_int, 16))

let parse_proto ident str =
  let list = String.split str ~on:'\t' in
  let proto, mask = String.lsplit2_exn (List.hd_exn list) ~on:'/' in
  Match.mask_ ident
    (Value.big_make (Bigint.of_string proto, 8))
    (Value.big_make (Bigint.of_string mask, 8))
...
let debug (q, w, e, r, t) =
  Match.to_string q ^ " " ^ Match.to_string w ^ " " ^ Match.to_string e ^ " "
  ^ Match.to_string r ^ " " ^ Match.to_string t
...
let parse_classbench fp =
  let rules =
    ...
    |> List.fold ~init:[] ~f:(fun rows line ->
           let vars = String.split line ~on:'\t' in
           let ip_src =
             List.nth_exn vars 0 |> parse_ip_mask "nw_src" |> Some
           in
           let ip_dst =
             List.nth_exn vars 1 |> parse_ip_mask "nw_dst" |> Some
           in
           let tcp_sport =
             List.nth_exn vars 2 |> parse_port_range "tp_src" |> Some
           in
           let tcp_dport =
...
let parse_of ident line : Match.t option =
  ...
  let nd =
    String.substr_index ~pos:st line ~pattern:","
  ...
  match ident with
  | "in_port" ->
      Value.big_make (Bigint.of_string data, 9) |> Match.exact_ ident
  | "nw_proto" ->
      Value.big_make (Bigint.of_string data, 8) |> Match.exact_ ident
  | "eth_type" | "tp_src" | "tp_dst" ->
      Value.big_make (Bigint.of_string data, 16) |> Match.exact_ ident
