...
let rec relabel (e : Expr.t) (sz : int) : Expr.t =
  ...
  | Value v when Value.size v >= 0 && Value.size v <> sz ->
      failwith
      @@ Printf.sprintf "Tried to relabel %s with #%d" (Value.to_string v) sz
  | (Var (_, sz') | Hole (_, sz')) when sz' >= 0 && sz <> sz' ->
      failwith
      @@ Printf.sprintf "Tried to relabel %s to width %d" (Expr.to_string e)
           sz
  ...
  | Cast (i, _) ->
      if i = sz then e
      else failwith "Tried to relabel an explicit cast incorrectly:("
  | Slice {hi; lo; _} ->
      let sz' = hi - lo in
...
let rec infer_expr (e : Expr.t) : Expr.t =
  ...
  let binop f (e1, e2) =
    ...
    assert (s1 >= -1) ;
    assert (s2 >= -1) ;
    if s1 = s2 then if s1 = -1 then f (infer_expr e1) (infer_expr e2) else e
    else if s1 = -1 then f (relabel e1 s2) e2
    else if s2 = -1 then f e1 (relabel e2 s1)
    else (
      Printf.printf "sizes are known and differ!\n\t%s\t%s\n%!"
        (Expr.to_string e1) (Expr.to_string e2) ;
      failwith "BitWidth type error" )
  in
  match e with
  | Value _ | Var _ | Hole _ -> e
...
let rec infer_test t =
  ...
  let binop_e op e1 e2 =
    ...
    if sz1 = -1 && sz2 >= 0 then op (relabel e1' sz2) e2'
    else if sz1 >= 0 && sz2 = -1 then op e1' (relabel e2' sz1)
    else if sz1 = sz2 then op e1' e2'
    else
      failwith
      @@ Printf.sprintf "[TypeError] %s and %s have different bitwidths"
           (Expr.to_string e1) (Expr.to_string e2)
  in
  match t with
  | True -> True
...
let rec infer (p : Cmd.t) : Cmd.t =
  ...
  | Select (typ, bs) ->
      select typ @@ List.map bs ~f:(fun (t, c) -> (infer_test t, infer c))
  | Apply t ->
