...
let edit_domain (prog : Cmd.t) (edit : Edit.t) =
  ...
  |> Option.value_exn |> table_vars

let edits_domain (prog : Cmd.t) (edits : Edit.t list) : (string * int) list =
  let open List in
  edits >>= edit_domain prog

let log_edit_domain problem =
  Problem.(edits_domain (log problem) (log_edits problem))

let make (problem : Problem.t) : t =
  List.fold
    (Problem.phys problem |> Cmd.get_tables_vars)
    ~init:Model.empty
    ~f:(fun acc (tbl, vars) ->
...
let optimization (params : Parameters.t) (problem : Problem.t) :
    Test.t -> Test.t =
