...
let delete_row_hole_name i tbl =
  Printf.sprintf "%s%d%s%s" delete_row_prefix i delete_row_infix tbl

let is_delete_hole n =
  String.(
    is_prefix ~prefix:delete_row_prefix n
...
let find_delete_row key =
  ...
  | Some idx_tbl -> (
    ...
    | Some idx ->
        ...
        let table_name =
          String.drop_prefix idx_tbl (idx + String.length delete_row_infix)
        in
...
let match_holes_mask tbl x =
  (match_hole_exact tbl x, Printf.sprintf "%s_mask" (match_hole_exact tbl x))
...
let match_holes_table encode_tag tbl keys =
  let open Test in
  List.fold keys ~init:True ~f:(fun acc k ->
      acc
      %&%
      match Cmd.Key.value k with
      | None ->
...
let action_data tbl i v sz =
  Printf.sprintf "%s%s_%d" (action_data_prefix tbl i) v sz
