...
let rec to_string (e : t) : string =
  let string_binop e1 op e2 = Printf.sprintf "(%s %s %s)" (to_string e1) op (to_string e2) in
  match e with
...
let rec to_sexp_string (e : t) =
  let string_binop ctor (e1, e2) = Printf.sprintf "%s(%s, %s)" ctor (to_sexp_string e1) (to_sexp_string e2) in
  match e with
  ...
  | Slice {hi;lo;bits} -> Printf.sprintf "Slice {hi=%d;lo=%d;bits=%s}" hi lo (to_sexp_string bits)

let rec equals e1 e2 =
  match (e1, e2) with
  | Value v1, Value v2 ->
     Value.equals v1 v2
  | Var (x1,sz1), Var (x2,sz2)
    | Hole (x1,sz1), Hole(x2,sz2) ->
  ...
  | Cast (sz1, e1), Cast(sz2,e2) ->
     sz1 = sz2 && equals e1 e2
  | Slice s1, Slice s2 ->
...
let rec holify ~f holes (e : t) : t =
  ...
  | Hole _ | Value _ -> e
  | Var (x,sz) ->
     begin match List.find holes ~f:(String.(=) x) with
     | None -> e
     | Some _ -> Hole (f (x, sz))
     end
