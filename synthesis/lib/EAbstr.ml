...
(** [abstract_data old_data new_data] returns a mapping from values in old_data
   to values in new_data. A pair [od ↦ nd] is added if the values are different
   and occur at the same place in the list and there isn't a previous pair in
   the map [od -> nd'] where [nd <> nd'] in which case it returns [None]. It also
   returns None if the lists are different lengths*)            
let abstract_data (old_data : Row.action_data) (new_data : Row.action_data) =
  List.fold2 old_data new_data
    ~init:(Some ValueMap.empty)
...
(** [abstract_matches old_matches new_matches] returns a mapping from values in
   old_matches to values in new_matches. A pair [om ↦ nm] is added if the values
   are different and occur at the same place in the list and there isn't a
   previous pair in the map [om -> nm'] where [nm <> nm'], in which case it
...
let abstract_matches (old_matches : Match.t list) (new_matches : Match.t list) =
  List.fold2 old_matches new_matches
    ~init:(Some MatchMap.empty)
    ~f:(fun acc_opt old_ new_ ->
      let%bind acc = acc_opt in
...
(** [sub_action_data map data] applies the mapping [map] to action data [data]*)  
...
(** [sub_action_data adata map e] applies the match mapping [map] and the data
   mapping adata to the edit [e]*)    
let sub_consts (adata : Value.t ValueMap.t option)
    (map : Match.t MatchMap.t) (e : Edit.t) : Edit.t option =
  match e with
  | Del _ -> None
  | Add (table, (ms, ad, idx)) ->
...
let infer (params : Parameters.t) (phys : Cmd.t) (e : Edit.t) =
  ...
  if List.length similars < min_match_size then
    None
  else
    get_cache ()
