...
let to_bmv2_string (cmd : Cmd.t) e =
  ...
  | Add (nm, (matches, action_data, action_id)) ->
      ...
      Printf.sprintf "table_add %s %s%s =>%s" nm act_name bmv2_matches
        bmv2_data
  | Del (t, i) -> Printf.sprintf "table_delete %s %d" t i
...
let read_vars cmd = function
  ...
  | Add (table, (ms, _, aid)) -> (
    ...
    | None -> failwith @@ Printf.sprintf "Couldn't find %s" table
    | Some (keys, actions, _) ->
        let reads = List.nth_exn actions aid |> action_reads in
        Match.relevant_keys ms |> StringSet.of_list |> StringSet.union reads
...
let write_vars cmd = function
  ...
  | Add (table, (_, _, aid)) -> (
    ...
    | None -> failwith @@ Printf.sprintf "Couldn't find %s" table
    | Some (_, actions, _) ->
        List.nth_exn actions aid |> trd3 |> Cmd.assigned_vars )

let to_model_alist phys e =
  match e with
  | Del (t, i) -> [(Hole.delete_row_hole_name i t, Value.make (1, 1))]
  | Add (t, (ms, ds, i)) -> (
    match Cmd.get_schema_of_table t phys with
    | None -> failwith @@ Printf.sprintf "Couldn't find table %s" t
    | Some (_, actions, _) ->
...
let to_model phys e = to_model_alist phys e |> Model.of_alist_exn

let list_to_model phys es =
  List.bind es ~f:(to_model_alist phys) |> Model.of_alist_exn

(** [negate phys m es] computes a test ensuring that [es] can never again be
    synthesized, if [es] is nonempty, otherwise, it negates the assignments
    in the model [m]. Assume that [Edit.extract phys m] = [t]. *)
    ...
    ~f:(fun ~key ~data acc ->
      ...
      | None -> (
        ...
        | Some (table_name, row_idx)
          when Bigint.(Value.get_bigint data = one) ->
