...
let quant_const expr etyp styp =
  match styp with
...
let rec expr_to_term expr styp : Smtlib.term =
  ...
  | Var (v, _) -> quant_const v "var" styp
  | Hole (h, _) -> quant_const h "hole" styp
...
let check (params : Parameters.t) typ (test : Ast.test) =
  ...
  let response = match typ with
    |`Sat -> assert_ prover (test_to_term test `Sat); check_sat_using (UFBV : tactic) prover
    |`Valid -> assert_ prover (not_ (test_to_term test `Valid)); check_sat_using (QFBV : tactic) prover in
