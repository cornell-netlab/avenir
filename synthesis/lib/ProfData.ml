...
let headers =
  ...
  ; "model_z3_calls"
  ; "num_backtracks"
    (*; ( "model_cond_time"; * "model_holes_time"; * "interp_time"; *
      "model_z3_calls"; * "fixup_time"; * "search_wp_time"; *
      "mean_tree_size"; * "min_tree_size"; * "max_tree_size"*) ]

let header_string = List.reduce_exn headers ~f:(fun x y -> x ^ "," ^ y)

let mean_tree_size data =
  if List.length !(data.tree_sizes) = 0 then 0
  else
    List.fold !(data.tree_sizes) ~init:0 ~f:( + )
    / List.length !(data.tree_sizes)

let max_tree_size data = List.fold !(data.tree_sizes) ~init:0 ~f:max

let min_tree_size data =
  List.fold !(data.tree_sizes) ~init:(max_tree_size data) ~f:min

let to_string (data : t) =
  Printf.sprintf
    "%d,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%d,%f,%f,%f,%f,%f,%d,%d"
    !(data.log_inst_size)
    (* data.phys_inst_size *)
    ...
    !(data.model_z3_calls) !(data.num_backtracks)
...
let update_time acc ?(end_time = Time.now ()) start_time =
  let open Time in
  diff end_time start_time |> update_time_val acc
