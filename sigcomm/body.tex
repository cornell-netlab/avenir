...
When a packet enters a switch, it will ``see'' exactly one sequence of
assignments. That is, absent cloning and multicast, each packet will
take either the true or false branch of each if statement, and hit
exactly one rule in a table. We call this sequence of assignments (and
the guards required to execute them) a \emph{trace}
$\sigma(\tau) \in \Cmd$. A trace is defined formally in
